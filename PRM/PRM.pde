...
//A box obstacle
static int numRects = 10;
Vec2 boxTopLefts[] = new Vec2[numRects];
...
void placeRandomObstacles(){
  ...
  for(int i = 0; i < numRects; i++){
     boxTopLefts[i] = new Vec2(random(50,950),random(50,700));
  }
}
...
void setup(){
  ...
  buildPRM(circlePos, circleRad, boxTopLefts, boxW, boxH);
  setGoal(closestNode(startPos),closestNode(goalPos));
  weightedPRM(circlePos, circleRad, boxTopLefts, boxW, boxH);
  runBFS();
}
...
void draw(){
  ...
  for(int i = 0; i < numRects; i++){
    rect(boxTopLefts[i].x, boxTopLefts[i].y, boxW, boxH);
  }
...
hitInfo rayBoxsListIntersect(Vec2[] boxTopLefts, float boxW, float boxH, Vec2 ray_start, Vec2 ray_dir, float max_t){
  hitInfo hit = new hitInfo();
  hit.t = max_t;
  for (int i = 0; i < numRects; i++){
...
void generateRandomNodes(Vec2[] circleCenters, float[] circleRadii, Vec2 []boxTopLefts, float boxW, float boxH){
  for (int i = 0; i < numNodes; i++){
    ...
    boolean insideBoxs = pointInBoxs(boxTopLefts,boxW,boxH,randPos);
    while (insideAnyCircle || insideBox || insideBoxs){
      ...
      insideBoxs = pointInBoxs(boxTopLefts,boxW,boxH,randPos);
    }
    nodePos[i] = randPos;
  }
}

void generateRandomNodesGoalCentric(Vec2[] circleCenters, float[] circleRadii, Vec2 []boxTopLefts, float boxW, float boxH){
    Vec2 start = nodePos[startNode];
    Vec2 goal = nodePos[goalNode];
    Vec2 stoG = goal.minus(start).normalized();
    Vec2 mid = goal.plus(start).times(0.5);
    nodePos[0] = start;
    nodePos[1] = goal;
    startNode = 0;
    goalNode = 1;
    ...
    for(int i = 2; i < numNodes; i++){
      Vec2 randPos = new Vec2(random(width),random(height));
      Vec2 ptoG = stoG.times(dot(randPos.minus(start),stoG));
      ptoG.add(start);
      ...
      while (insideAnyCircle || insideBox || insideBoxs){
        ...
        ptoG = stoG.times(dot(randPos.minus(start),stoG));
        ptoG.add(start);
        ptoG.subtract(randPos);
        randPos.add(new Vec2(random(ptoG.x/2)+ptoG.x/4,random(ptoG.y/2)+ptoG.y/4));
      ...
      nodePos[i] = randPos;
      println("Distance to line: (",ptoG.x,", ",ptoG.y,")");
      println("Final randPos: (",randPos.x,", ",randPos.y,")");
    }

}

void setGoal(int startID, int goalID){
  startNode = startID;
  goalNode = goalID;
}
  ...
  for (int i = 0; i < numNodes; i++){
    ...
    for (int j = 0; j < numNodes; j++){
      ...
      hitInfo boxCheck = rayBoxIntersect(boxTopLeft,boxW,boxH,nodePos[i],dir,distBetween);
       hitInfo boxsCheck = rayBoxsListIntersect(boxTopLefts,boxW,boxH,nodePos[i],dir,distBetween);
      if (!boxCheck.hit && !circleListCheck.hit && !boxsCheck.hit && distBetween < 200){
        neighbors[i].add(j);
      }
    }
  }
...
void buildPRM(Vec2[] circleCenters, float[] circleRadii, Vec2 []boxTopLefts, float boxW, float boxH){
  generateRandomNodes(circleCenters, circleRadii, boxTopLefts, boxW, boxH);
  connectNeighbors();
}

void weightedPRM(Vec2[] circleCenters, float[] circleRadii, Vec2 []boxTopLefts, float boxW, float boxH){
  generateRandomNodesGoalCentric(circleCenters, circleRadii, boxTopLefts, boxW, boxH);
  connectNeighbors();
}
